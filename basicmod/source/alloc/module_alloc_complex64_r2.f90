module module_alloc_complex64_r2_bmod
!! License: GNU GPLv2 license.
!! Author: Francisco Pena, fran.pena(at)usc.es
!! Date: 31/12/2017
!!
!! Module for memory allocation of complex(real64) rank 2 allocatable arrays.  
!!
...
!! This is why pairs like `[[module_alloc_complex64_r2_bmod(module):find(interface)]]` and 
!! `[[module_alloc_complex64_r2_bmod(module):sfind(interface)]]` have different public names.  
!!
!! @warning The Fortran 2003 standard implements the clause `source` to allocate an array (included in ifort since version 13.0). 
!! Thus, since `[[module_alloc_complex64_r2_bmod(module):find(interface)]]` returns an array, this code is correct:  
...
!! `[[module_alloc_complex64_r2_bmod(module):sfind(interface)]]`.  
!
! PUBLIC PROCEDURES:
...
private :: insert_section_prv, find_sca_prv, find_vec_prv, sfind_sca_prv, sfind_vec_prv
private :: search_multiple

!Interfaces
interface dealloc
  !! Deallocates a complex64 array of rank 2.  
  !!
...
interface alloc
  !! Allocates a complex64 array of rank 2.  
  !!
...
if (res /= 0) call error('(module_alloc_complex64_r2/dealloc) Unable to deallocate variable: '//trim(cad))
end subroutine
...
if (allocated(v)) then
  if (size(v,1) == d1 .and. size(v,2) == d2) then; v = (0._real64, 0._real64); return; end if
  call dealloc(v)
end if
allocate(v(d1, d2), stat = res, errmsg = cad)
if (res /= 0) call error('(module_alloc_complex64_r2/alloc) unable to allocate variable: '//trim(cad))
...
!! `end program`  
complex(real64), allocatable  :: v(:,:) !! Array to change dimensions.
...
if (.not. allocated(v)) then
  ...
  if (res /= 0) call error('(module_alloc_complex64_r2/extend) unable to allocate variable v: '//trim(cad))
  v = (0._real64, 0._real64)
  ...
  if (d1 > s1 .or. d2 > s2) then !reallocation is mandatory
    ...
    if (res /= 0) call error('(module_alloc_complex64_r2/extend) unable to allocate variable temp: '//trim(cad))
    temp = (0._real64, 0._real64)
    temp(1:s1,1:s2) = v
...
!! `call reduce(v, 20, 20)`  
!! `end program`  
complex(real64), allocatable :: v(:,:)   !! Array to reduce.
integer, intent(in)          :: d1       !! Array extension in the first dimension.
integer, intent(in)          :: d2       !! Array extension in the second dimension.
complex(real64), allocatable :: temp(:,:)

!if (d1 == 0 .or. d2 == 0) then
!  call info('(module_alloc_complex64_r2/reduce) Some given dimension is zero, variable will be deallocated')
!  call dealloc(v)
!else
if (.not. allocated(v)) then
  call info('(module_alloc_complex64_r2/reduce) Variable not allocated'); return
end if
...
if (size(v,1) <  d1 .or.  size(v,2) <  d2) then   !rows or cols are too large
  call info('(module_alloc_complex64_r2/reduce) Some given dimension is too large to reduce'); return
end if
...
!! `implicit none`  
!! `complex(real64), allocatable :: v(:,:)`  
!! `call set(v, (0._real64, 0._real64), 10, 20, fit=[.true.,.false.])`  
!! `end program`  
...
!! `implicit none`  
!! `complex(real64), allocatable :: v(:,:)`  
!! `call set(1, v, [(0._real64, 1._real64),(2._real64, 0._real64),(0._real64, 3._real64)], 10, fit=[.true.,.false.])`  
!! `end program`  
integer,           intent(in) :: dimen  !! Dimension to work with.
complex(real64), allocatable  :: v(:,:) !! Array.
complex(real64),   intent(in) :: val(:) !! Section values.
...
case default
  call error('(module_alloc_complex64_r2/set_section_prv) 1st argument dimen must be 1 or 2.')
end select
end subroutine
...
!! `implicit none`  
!! `complex(real64), allocatable :: v(:,:)`  
!! `call set(v, reshape([(0._real64, 1._real64),(2._real64, 0._real64), (1._real64, 1._real64),(2._real64, 3._real64)], &`  
!! `[2,2]), [10,20], [1,3], fit=[.true.,.false.])`  
!! `end program`  
complex(real64), allocatable  :: v(:,:)   !! Array.
complex(real64), intent(in)   :: val(:,:) !! Values.
...
!! `implicit none`  
!! `complex(real64), allocatable :: v(:,:)`  
!! `call add(v, (0._real64, 1._real64), 10, 20, fit=[.true.,.false.])`  
!! `end program`  
complex(real64), allocatable  :: v(:,:) !! Array.
complex(real64), intent(in)   :: val    !! Value.
integer, intent(in)           :: d1     !! Position in the first dimension.
...
!! `implicit none`  
!! `complex(real64), allocatable :: v(:,:)`  
!! `call add(1, v, [(0._real64, 1._real64),(2._real64, 0._real64), (1._real64, 1._real64),(2._real64, 3._real64)], & `  
!! `10, fit=[.true.,.false.])`  
!! `end program`  
integer,           intent(in) :: dimen  !! Dimension to work with.
complex(real64), allocatable  :: v(:,:) !! Array.
...
!! `implicit none`  
!! `complex(real64), allocatable :: v(:,:)`  
!! `call add(v, reshape([(0._real64, 1._real64),(2._real64, 0._real64), (1._real64, 1._real64),(2._real64, 3._real64)], &`  
!! `[2,2]), [10,20], [1,3], fit=[.true.,.false.])`  
!! `end program`  
complex(real64), allocatable  :: v(:,:)   !! Array.
complex(real64),   intent(in) :: val(:,:) !! Values.
...
!! `implicit none`  
!! `complex(real64), allocatable :: v(:,:)`  
!! `call insert(1, v, [(0._real64, 1._real64),(2._real64, 0._real64), (1._real64, 1._real64),(2._real64, 3._real64)], &`  
!! `10, used=3, fit=[.true.,.false.])`  
!! `end program`  
integer,           intent(in) :: dimen  !! Dimension to work with.
complex(real64), allocatable  :: v(:,:) !! Array.
complex(real64),   intent(in) :: val(:) !! Section values.
...
case (1) ! Row
  ...
  v(d,                       1:size(val,1))           = val
  v(d,                       size(val,1)+1:size(v,2)) = (0._real64, 0._real64)
case (2) ! Column
  call extend(v, size(val,1), s, fit)
  v(:,                       d+1:size(v,2))           = v(:, d:size(v,dimen)-1)
  v(1:size(val,1),           d)                       = val
  v(size(val,1)+1:size(v,1), d)                       = (0._real64, 0._real64)
case default
  call error('(module_alloc_complex64_r2/insert_section_prv) 1st argument dimen must be 1 or 2.')
end select
end subroutine
...
!! `implicit none`  
!! `integer :: i`  
!! `complex(real64) :: v(4,4) = reshape([(cmplx(i,i,real64), i=1,16)], [4,4])`  
!! `integer, allocatable :: res(:,:)`  
!! `call sfind(v, (2._real64, 2._real64), res)`  
!! `end program`  
...
if (csize(cmplx2=v, d=1) <= 0 .or. csize(cmplx2=v, d=2) <= 0) return
! allocate row, col
...
!! `implicit none`  
!! `integer:: i`  
!! `real(real64) :: v(4,4) = reshape([(cmplx(i,i,real64), i=1,16)], [4,4])`  
!! `integer, allocatable :: res(:,:)`  
!! `call sfind(v, [((1._real64,1._real64)*i, i=1,8)], res)`  
!! `end program`  
complex(real64),      intent(in)  :: v(:,:)   !! Array.
...
if (csize(cmplx2=v, d=1) <= 0 .or. csize(cmplx2=v, d=2) <= 0 .or. csize(cmplx1=val, d=1) <= 0) return
! allocate row, col
n = 0
do j = 1, size(val,1)
...
!! `implicit none`  
!! `integer :: i`  
!! `complex(real64) :: v(4,4) = reshape([((i,i,real64), i=1,16)], [4,4])`  
!! `integer, allocatable :: pos(:,:)`  
!! `allocate(pos, source=find(v, (4._real64,4._real64))`  
!! `! the latter is not valid in Gfortan 4.9 and below; write the following code or use sfind instead:`  
!! `! pos = find(v, (4._real64,4._real64))`  
!! `end program`  
complex(real64), intent(in) :: v(:,:)   !! Array where the search is done.
complex(real64), intent(in) :: val      !! Value to search.
...
!! `! the latter is not valid in Gfortan 4.9 and below; write the following code or use sfind instead:`  
!! `! pos = find(v, [((1._real64,1._real64)*i, i=1,3)], res)`  
!! `end program`  
complex(real64), intent(in) :: v(:,:)   !! Array where the search is done.
complex(real64), intent(in) :: val(:)   !! Values to search.
integer, allocatable     :: res(:,:) !! Array of positions; a zero-sized array if nothing was found.
