module module_alloc_int_r3_bmod
!! License: GNU GPLv2 license.
!! Author: Francisco Pena, fran.pena(at)usc.es
!! Date: 31/12/2017
!!
!! Module for memory allocation of integer rank 3 allocatable arrays.  
!!
...
case default
  call error('(module_alloc_int_r2/add_section_prv) 1st argument dimen must be 1 or 2.')
end select
end subroutine
...
!! `implicit none`  
!! `integer :: i`  
!! `integer :: v(4,4,4) = reshape([(i, i=1,64)], [4,4,4])`  
...
if (csize(int3=v, d=1) <= 0 .or. csize(int3=v, d=2) <= 0 .or. csize(int3=v, d=3) <= 0) return
! allocate row, col
...
!! `implicit none`  
!! `integer :: i`  
!! `integer :: v(4,4,4) = reshape([(i, i=1,64)], [4,4,4]), val(8) = [(2*i, i=1,8)]`  
!! `integer, allocatable :: res(:,:)`  
