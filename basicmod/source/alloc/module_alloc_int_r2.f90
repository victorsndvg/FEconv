module module_alloc_int_r2_bmod
...
!! This is why pairs like `[[module_alloc_int_r2_bmod(module):find(interface)]]` and   
!! `[[module_alloc_int_r2_bmod(module):sfind(interface)]]` have different public names.  
!! 
!! @warning The Fortran 2003 standard implements the clause `source` to allocate an array (included in ifort since version 13.0). 
!! Thus, since `[[module_alloc_int_r2_bmod(module):find(interface)]]` returns an array, this code is correct:  
...
!! `[[module_alloc_int_r2_bmod(module):sfind(interface)]]`.  
!
! PUBLIC PROCEDURES:
...
use module_alloc_int_r1_bmod, only: alloc, bsearch
...
!! `call dealloc(v)`  
!! `end program`  
integer, allocatable :: v(:,:)  !! Array to deallocate.
integer :: res
...
!! `call alloc(v, 20, 30)`  
!! `end program`  
integer, allocatable :: v(:,:) !! Array to allocate.
...
!! `end program`  
integer, allocatable          :: v(:,:) !! Array to change dimensions.
...
!! `call reduce(v, 20, 20)`  
!! `end program`  
integer, allocatable :: v(:,:)   !! Array to reduce.
integer, intent(in)  :: d1       !! Array extension in the first dimension.
integer, intent(in)  :: d2       !! Array extension in the second dimension.
integer, allocatable :: temp(:,:)

!if (d1 == 0 .or. d2 == 0) then
!  call info('(module_alloc_int_r2/reduce) Some given dimension is zero, variable will be deallocated')
!  call dealloc(v)
!else
if (.not. allocated(v)) then
...
integer, allocatable          :: v(:,:) !! Array.
integer, intent(in)           :: val(:) !! Section values.
...
case default
  call error('(module_alloc_int_r2/set_section_prv) 1st argument dimen must be 1 or 2.')
end select
end subroutine
...
!! `implicit none`  
!! `integer, allocatable :: v(:,:)`  
!! `call add(v, 25, 10, 20, fit=[.true.,.false.])`  
!! `end program`  
integer, allocatable :: v(:,:)          !! Array.
integer, intent(in)  :: val             !! Value.
integer, intent(in)  :: d1              !! Position in the first dimension.
...
!! `implicit none`  
!! `integer, allocatable :: v(:,:)`  
!! `call add(1, v, [25,35,45,55], 10, fit=[.true.,.false.])`  
!! `end program`  
integer, intent(in)           :: dimen  !! Dimension to work with.
integer, allocatable          :: v(:,:) !! Array.
integer, intent(in)           :: val(:) !! Section values.
...
!! `implicit none`  
!! `integer, allocatable :: v(:,:)`  
!! `call add(v, reshape([25,35,45,55],[2,2]), [10,20], [1,3], fit=[.true.,.false.])`  
!! `end program`  
integer, allocatable :: v(:,:)          !! Array.
integer, intent(in)  :: val(:,:)        !! Values.
integer, intent(in)  :: d1(:)           !! Positions in the first dimension.
...
integer, intent(in)           :: val(:) !! Section values.
integer, intent(in)           :: d      !! Position in the given dimension of the section to insert.
integer, intent(in), optional :: used   !! Number of sections actually used in vector.
logical, intent(in), optional :: fit(2) !! Whether to fit the array extension to the maximum position; by default, `.true.`
integer :: s
...
case default
  call error('(module_alloc_int_r2/insert_section_prv) 1st argument dimen must be 1 or 2.')
end select
end subroutine
...
!! `integer :: i`  
!! `call alloc(v, 4, 4)`  
!! `v = reshape([(i, i=1,16)], [4,4])`  
!! `call insert_sorted(1, v, [(3*i, i=1,4)])`  
!! `end program`  
integer,           intent(in)    :: dimen  !! Dimension to work with.
integer, allocatable             :: v(:,:) !! Array.
integer,           intent(in)    :: val(:) !! Section values.
...
case default
  call error('(module_alloc_int_r2/insert_section_sorted_prv) 1st argument dimen must be 1 or 2.')
end select
if (present(pos)) pos = indx
end subroutine
...
!! `implicit none`  
!! `integer :: i`  
!! `integer :: v(4,4) = reshape([(i, i=1,16)], [4,4])`  
!! `integer, allocatable :: res(:,:)`  
!! `call sfind(v, 2, res)`  
!! `end program`  
integer,              intent(in)  :: v(:,:)   !! Array.
...
if (csize(int2=v, d=1) <= 0 .or. csize(int2=v, d=2) <= 0) return
! allocate row, col
...
!! `integer :: v(4,4) = reshape([(i, i=1,16)], [4,4])`  
!! `integer, allocatable :: res(:,:)`  
!! `call sfind(v, [(2*i, i=1,8)], res)`  
!! `end program`  
integer,              intent(in)  :: v(:,:)   !! Array.
integer,              intent(in)  :: val(:)   !! Values to search.
integer, allocatable, intent(out) :: res(:,:) !! Array of positions; a zero-sized array if nothing was found.
integer :: i, j, k, n, p

if (csize(int2=v, d=1) <= 0 .or. csize(int2=v, d=2) <= 0 .or. csize(int1=val, d=1) <= 0) return
! allocate row, col
n = 0
do j = 1, size(val,1)
...
!! `! pos = find(v, 4)`  
!! `end program`  
integer, intent(in)  :: v(:,:)   !! Array where the search is done.
integer, intent(in)  :: val      !! Value to search.
...
!! `call alloc(v, 4, 4)`  
!! `v = reshape([(i, i=1,16)], [4,4])`  
!! `print*, find_sorted(1, v, [3, 7, 11, 15])`  
!! `end program`  
integer, intent(in)           :: dimen  !! Dimension to work with.
integer, intent(in)           :: v(:,:) !! Array.
integer, intent(in)           :: val(:) !! Section values.
...
case default
  call error('(module_alloc_int_r2/find_section_sorted_prv) 1st argument dimen must be 1 or 2.')
end select
end function
end module
