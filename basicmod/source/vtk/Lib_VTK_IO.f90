...
!----------------------------------------------------------------------------------------------------------------------------------
USE IR_Precision_bmod ! Real and integer portable multi-precision kind definition.
!----------------------------------------------------------------------------------------------------------------------------------
...
! FRAN: packing in ASCII files
character(len=1024)::      line = ' '                 ! string to store packed records in ascci files
character(len=1024)::      strn  = ' '                 ! auxiliary string to store packed records in ascci files
!-----------------------------------------------------------------------------------------------------------------------------------
...
contains
  ...
  case(f_out_ascii)
    ...
    write(unit=Unit_VTK,fmt='(A)',iostat=E_IO)repeat(' ',indent)// &
                                              '<DataArray type="Float64" NumberOfComponents="3" Name="Point" format="ascii">'
    !FRAN write(unit=Unit_VTK,fmt='(3'//FR8P//')',iostat=E_IO)(X(n1),Y(n1),Z(n1),n1=1,NN)
  ...
  case(f_out_ascii)
    ...
    !FRAN write(unit=Unit_VTK,fmt=FI4P, iostat=E_IO)(connect(n1),n1=1,size(connect))
    ...
    !FRAN write(unit=Unit_VTK,fmt=FI4P, iostat=E_IO)(offset(n1),n1=1,NC)
    ...
    !FRAN write(unit=Unit_VTK,fmt=FI1P, iostat=E_IO)(cell_type(n1),n1=1,NC)
  ...
  case(f_out_ascii)
    ...
    case('NODE')
      select case(trim(Upper_Case(var_block_action)))
      case('OPEN')
        write(unit=Unit_VTK,fmt='(A)',iostat=E_IO)repeat(' ',indent)//'<PointData'//trim(activef)//'>'
        indent = indent + 2
      case('CLOSE')
        indent = indent - 2
